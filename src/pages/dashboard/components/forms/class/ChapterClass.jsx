...
const ChapterClass = ({ refetch, courseId, onClose }) => {
  const { id } = useParams();
  ...
  const uploadImage = async (files) => {
    ...
    try {
      ...
      if (
        ...
        console.log("Uploaded Images Data:", response.data);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      console.log(values);
      try {
        const response = await axiosSecure.post("/class/add", {
        ...
        if (response.status === 201) {
          console.log("New Class added successfully:", values);
    ...
    <form className="space-y-4 " onSubmit={formik.handleSubmit}>
      ...
      <div className="flex items-center flex-col md:flex-row gap-2">
        ...
        <div className="w-full flex-1">
          ...
          <Select
            className="custom-select w-full h-[40px] rounded-sm bg-[#FBFDFC] border border-[#E6E6E6] mt-0"
            components={{ DropdownIndicator }}
          ...
          {formik.values.practiceSheet &&
            formik.values.practiceSheet.size > 0 && (
              <p className="text-sm mt-1">
                ...
                {(formik.values.practiceSheet.size / 1024 / 1024).toFixed(2)} MB
              </p>
            )}
