...
const NoticeUpdate = ({ noticeData, refetch, onClose }) => {
  ...
  const { data: item } = useQuery({
    queryKey: ["singlenotice", noticeData],
    queryFn: async () => {
      try {
        const res = await axiosSecure.get(`/notice/${noticeData}`);
        res.data;
  ...
  useEffect(() => {
    ...
    if (courseOptions && item?.courses) {
      const selected = courseOptions.filter((course) =>
        item.courses.includes(course.value)
      );
  ...
  }, [courseOptions, item?.courses]); // Adjusted dependency array
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await useAxiosSecure.patch(`/notice/${noticeData}`, {
          title: values.title,
        ...
        if (response.status === 200) {
          " Notice Updated successfully:", values;
  ...
  useEffect(() => {
    if (item) {
      formik.setValues({
        ...
        course: item?.courses.map((course) => course),
        uploadedFeaturedImageUrl: item?.featuredImgUrl,
        uploadedFileUrls: item?.files,
      });

      SetSelectedOptions(item?.courses.map((course) => course));
    }
  }, [item]);
  ...
  //   formik.setFieldValue("uploadedFeaturedImageUrl", "");
  // };
      ...
      <form className="" onSubmit={formik.handleSubmit}>
        ...
        <div className="mt-3">
          ...
          <Select
            ...
            onChange={(selectedOptions) => {
              ...
              setSelectedCourse(selectedOptions); // Update Select component state
        ...
        {item?.featuredImgUrl && (
          <div className="relative w-24 h-16 ">
            <img
              src={item?.featuredImgUrl || "https://via.placeholder.com/150"}
              alt="Image"
        ...
        <div className="mt-3">
          ...
          <label
            htmlFor="files"
            className="border border-dashed border-gray-300 p-2 rounded-md text-center cursor-pointer flex flex-col items-center justify-center gap-2 mt-2 py-5"
          >
            <span className="bg-gray-200 dark:text-gray-800 p-3 rounded-full">
              <FileUp />
            </span>
            ...
            <Input
              ...
              multiple
              className="mt-1 h-full w-full border-dashed  hidden"
