...
const BookList = () => {
    ...
    const [bookData, bookRefetch, bookLoading] = useBookData()
    ...
    const handleDelete = async (id) => {
        setIsDeleting(true);
        try {
            const response = await axiosSecure.delete(`/book/${id}`);
    ...
    return (
        <div>
            <CustomMetaTag title={"Book List"} />
            <div className=" flex items-center lg:flex-row flex-col lg:gap-0 gap-5 justify-between">
                <h1 className="text-2xl text-gray-600">Book Management</h1>
                ...
                <div className="p-4 md:px-6 py-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-1 space-y-2">
                    <div>
                        <div className="flex items-center gap-2 ">
                            <h1 className="text-lg font-bold">Total Books</h1>
                            <span className="bg-gray-100 dark:text-gray-700 px-2 rounded-3xl text-sm font-semibold">
                                {bookData?.length} Total
                            </span>
                        </div>
                        <p className="text-gray-600 ">
                            You can manage Book here seamlessly
                        </p>
                    </div>
                </div>
                                                                ...
                                                                >
                                                                    <DialogTrigger asChild>
                                                                        <Pen
                                                                            ...
                                                                            onClick={() =>
                                                                                handleUpdateModalOpen(book._id)
                                                                            }
                                                                        />
                                                                    </DialogTrigger>
                                                                    <DialogContent className="p-8 min-w-[45%] max-h-screen overflow-auto">
